...
export default function JoinPage() {
  ...
  const handleJoin = async () => {
    ...
    setIsLoading(true);
    setTimeout(() => {
      const path = isPublisher ? "host" : "participant";
      router.push(`/room/${path}?name=${encodeURIComponent(identity)}&room=${encodeURIComponent(roomName)}`);
    }, 1000);
  };
        ...
        <div className="bg-white/10 backdrop-blur-lg rounded-2xl shadow-2xl border border-white/20 p-8 space-y-6 transform transition-all duration-500 hover:scale-105">
          ...
          <div className="space-y-4">
            ...
            <div className="group">
              ...
              <div className="relative">
                ...
                <input
                  ...
                  value={identity}
                  onChange={(e) => setIdentity(e.target.value)}
                  className="w-full pl-10 pr-4 py-3 bg-white/5 border border-white/20 rounded-lg text-white placeholder-white/40 focus:outline-none focus:ring-2 focus:ring-purple-400 focus:border-transparent transition-all duration-200"
                />
              </div>
            </div>
            ...
            <div className="space-y-3">
              ...
              <div className="flex space-x-4">
                {/* Publisher Option */}
                <label className={`flex-1 relative cursor-pointer ${isPublisher ? 'text-white' : 'text-white/60'}`}>
                  <input type="radio" name="role" checked={isPublisher} onChange={() => setIsPublisher(true)} className="sr-only" />
                  <div className={`p-4 rounded-lg border-2 ${isPublisher ? 'border-purple-400 bg-purple-500/20 shadow-lg' : 'border-white/20 bg-white/5 hover:border-white/40'}`}>
                    <div className="flex items-center space-x-3">
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${isPublisher ? 'border-purple-400' : 'border-white/40'}`}>
                        {isPublisher && <div className="w-2 h-2 bg-purple-400 rounded-full"></div>}
                      </div>
                      <div>
                ...
                {/* Audience Option */}
                <label className={`flex-1 relative cursor-pointer ${!isPublisher ? 'text-white' : 'text-white/60'}`}>
                  <input type="radio" name="role" checked={!isPublisher} onChange={() => setIsPublisher(false)} className="sr-only" />
                  <div className={`p-4 rounded-lg border-2 ${!isPublisher ? 'border-purple-400 bg-purple-500/20 shadow-lg' : 'border-white/20 bg-white/5 hover:border-white/40'}`}>
                    <div className="flex items-center space-x-3">
                      <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center ${!isPublisher ? 'border-purple-400' : 'border-white/40'}`}>
                        {!isPublisher && <div className="w-2 h-2 bg-purple-400 rounded-full"></div>}
                      ...
                      <div>
                        ...
                        <div className="text-xs text-white/60">Mic only (no camera/screen)</div>
                      </div>
                    </div>
                  </div>
                </label>
          ...
          <button
            onClick={handleJoin}
            disabled={isLoading || !identity || !roomName}
            className="w-full py-4 px-6 bg-gradient-to-r from-purple-500 to-blue-500 text-white font-semibold rounded-lg shadow-lg hover:shadow-xl transform transition-all duration-200 hover:scale-105 disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isLoading ? (
              <div className="flex items-center justify-center space-x-2">
                <div className="w-5 h-5 border-2 border-white/30 border-t-white rounded-full animate-spin"></div>
